class EstadisticasAgostoSeptiembre2010 < ActiveRecord::Migration
  def self.up
    ...
    " gross_salary > 3000 and gross_salary < 60000 and rubro_empresa <> 'IBM' and (extract(month from timestamp fecha_calculo)+1) = 8  " +
    ...
    " gross_salary > 3000 and gross_salary < 60000 and cargo <> 'IBM' and (extract(month from timestamp fecha_calculo)+1) = 8 " + 
    ...
    execute "INSERT INTO estadistica_por_rubro_mes  (rubro_empresa, salario_promedio, mes,anio)  " +
     ...
     " where ms.salary_id = s.id and ms.month like '%mayo%' and rubro_empresa <> 'ninguno' and rubro_empresa <> 'ninguna' and rubro_empresa <> 'niguno'   and " +
    " gross_salary > 3000 and gross_salary < 60000 and rubro_empresa <> 'IBM' and (extract(month from timestamp fecha_calculo)+1) = 9  " +
    "  group by rubro_empresa   having count(*) > 10 "
    ...
    " where ms.salary_id = s.id and ms.month like '%mayo%' and cargo <> 'ninguno' and cargo <> 'ninguna' and cargo <> 'niguno'  and " + 
    " gross_salary > 3000 and gross_salary < 60000 and cargo <> 'IBM' and (extract(month from timestamp fecha_calculo)+1) = 9 " + 
    " group by cargo having count(*) > 3 "
